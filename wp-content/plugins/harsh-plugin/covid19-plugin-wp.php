...
/**
* Plugin Name: COVID-19 - Spreems - Harsh
* Description: API driven site by Spreems Inc. | The plugin allows adding statistics table/widgets via shortcode to inform site visitors about changes in the situation about Coronavirus pandemic.
* Version: 2.2.3
* Author: Harsh suthar
* Author URI: https://www.developerharsh.com/
* Requires PHP: 5.6.20
...
if (!class_exists('CovidNycreatis')) {
	class CovidNycreatis
	{
		...
		function __construct(){
			define('COVID_NYCREATIS_VER', '2.2.6.1');
			...
			add_action( 'wp_enqueue_scripts', 'covid_city' );
			add_action("wp_ajax_city_data",array($this,  "get_city_data"));
			$this->wp_parse_args();
			$this->nycreatisDL();
			...
			add_shortcode('COVID19-SHEET-CITY', array($this, 'nycreatis_short_sheet_city'));
		...
		function covid_city(){
			wp_localize_script( 'my_city_script', 'myAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));      
		}
		...
		function ncrtsGen_backup($countries = false, $story = false){
			 
			$ncrtsURI='http://40.86.219.37:8040/services/Covid19/';
			 
			$ncrtsTrack='data/total/world';
			if ($story) {$ncrtsTrack = 'data/historical/world';} 
			if ($countries && !$story) {$ncrtsTrack = 'data/total/';} 
			else if ($story && $countries) {$ncrtsTrack = 'data/historical/'.$countries.'?lastdays=60';} 
		...
		function ncrtsGen($countries = false, $story = false){
			if(!isset($_GET['country']) || !isset($_GET['fromdate']) || !isset($_GET['todate']))
			{
				return $this->ncrtsGen_backup($countries, $story);
			}

			$country = $_GET['country'] === 'USA'? 'US': $_GET['country'];
			$fromdate = date('m-d-Y', strtotime($_GET['fromdate']));
			$todate = date('m-d-Y', strtotime($_GET['todate']));

			$ncrtsURI="http://40.86.219.37:8040/services/Covid19/data/historical/{$country}?fromdate={$fromdate}&todate={$todate}";
			//var_dump($ncrtsURI);
			$args = array('timeout' => 120); 
		...
		function nycreatis_short_sheet_city($atts){
			...
			echo $this->render_sheet_city($params);
			return ob_get_clean();
		}
		...
		function render_graph($params, $data){
			...
			?>
			<?php $allCountries = get_option('nycreatisCC'); ?>

			<form>
			<div class="row">
				<div class="col-md-4">
					<label for="country">Choose Country 
						<!-- <pre> -->
						<select class="form-control" id="country" name="country">
							<?php $sc = isset($_GET['country'])? $_GET['country']: 'Canada'; ?>
							<?php
							foreach ($allCountries as $item) {
								// var_dump($item);
								// die;
								$checked = $sc ===  $item->country? 'selected': '';

								echo '<option data-test="' . $sc . ' - ' .  $item->country . '" value="' . $item->country . '" ' . $checked . '>' . nycreatisCN($item) . '</option>';
							} ?>
						</select>
					</label>
				</div>
				<div class="col-md-4">
					...
					<label for="fromdate">From Date
						<input type="date" class="form-control" id="fromdate" name="fromdate" value="<?= isset($_GET['fromdate']) ? $_GET['fromdate']: date('Y-m-d', strtotime('-30 days')); ?>">
					</label>
				...
				<div class="col-md-4">
					...
					<label for="todate">To Date
						<input type="date" class="form-control" id="todate" name="todate" value="<?= isset($_GET['todate']) ? $_GET['todate']: date('Y-m-d'); ?>">
					</label>
			...
			<div class="ie covid19-graph <?php echo $all_options['cov_theme']; ?> <?php if ((isset($all_options['cov_rtl']) ? $all_options['cov_rtl'] : null) == 'on') echo 'rtl_enable'; ?>" style="position:relative;font-family:<?php echo $all_options['cov_font']; ?>">
				<span class="covid19-graph-title"><?php esc_attr_e($params['title']); ?></span>
		...
		function render_slip($params, $data){
			ob_start();
			include(COVID_NYCREATIS_PATH . 'includes/render_slip.php');
			return ob_get_clean();
		}
		...
		function render_widget($params, $data){
			...
			<div class="covid19-card <?php echo $all_options['cov_theme']; ?> <?php if ((isset($all_options['cov_rtl']) ? $all_options['cov_rtl'] : null) == 'on') echo 'rtl_enable'; ?>" style="font-family:<?php echo $all_options['cov_font']; ?>">
				...
				<?php if (isset($data->countryInfo->flag)) : ?><!--<span class="r-country_flag" style="background:url(<?php echo esc_html($data->countryInfo->flag); ?>) center no-repeat;background-size:cover;"></span>-->
				<?php endif; ?>
				<?php echo esc_html(isset($params['title_widget']) ? $params['title_widget'] : ''); ?></h4>
					...
					foreach ($data as $key => $value) : if ($nyc++ > 29) break; ?>
						<div class="covid19-creep-country">
							<div class="covid19-creep-col covid19-creep-countrycol">
								...
								<?php endif; ?>
								<span class="covid19-creep-col-country"><?php echo esc_html(nycreatisCN($value)); ?></span>
						...
						<li class="covid19-country">
							<div class="covid19-country-stats">
								<div class="covid19-col covid19-countrycol">
									...
									<?php endif; ?>
									<?php echo esc_html(nycreatisCN($value)); ?>
								</div>
								<div class="covid19-col covid19-confirmed">
		...
		?>
			<div class="table100 ver1 <?php echo $all_options['cov_theme']; ?> <?php if ((isset($all_options['cov_rtl']) ? $all_options['cov_rtl'] : null) == 'on') echo 'rtl_enable'; ?>" style="font-family:<?php echo $all_options['cov_font']; ?>">
				<div class="covid19-sheet table100-nextcols">
					<table class="nowrap userflags" id="<?php echo esc_attr($uniqId); ?>" data-page-length="<?php echo esc_attr($params['rows']); ?>" role="grid" style="width:100%" width="100%">
						<thead>
						...
						<tbody>
							<?php foreach ($data as $key => $value) : ?>
								<tr class="row100 body">
									<td class="cell100 column2 Ncrts-<?php $arr = explode(' ', trim($value->country)); echo $arr[0]; ?> country_title" data-label="<?php echo esc_html($params['country_title']); ?>" title="<?php echo esc_html(nycreatisCN($value)); ?>" ncrts-country="<?php echo esc_html($value->country); ?>">
										<?php if (isset($value->countryInfo->flag)) : ?>
											<span class="country_flag" style="background:url(<?php echo esc_html($value->countryInfo->flag); ?>) center no-repeat;background-size:cover;"></span>
										<?php endif; ?>
										<?php echo esc_html(nycreatisCN($value)); ?></td>
		...
		function get_city_data(){
			echo file_get_contents('http://40.86.219.37:8040/services/Covid19/citydata/total/US/cities/'.$_REQUEST['city']);
			wp_die( );
		}
		...
		function render_sheet_city($params){
			$data = [];
			wp_enqueue_style('covid');
			wp_enqueue_style('jquery.datatables');
			wp_enqueue_script('jquery.datatables');
		...
		?>
			<div class="table100 ver1 <?php echo $all_options['cov_theme']; ?> <?php if ((isset($all_options['cov_rtl']) ? $all_options['cov_rtl'] : null) == 'on') echo 'rtl_enable'; ?>" style="font-family:<?php echo $all_options['cov_font']; ?>">
				<div class="covid19-sheet table100-nextcols">
					...
					<table class="nowrap userflags chan" id="<?php echo esc_attr($uniqId); ?>" data-page-length="<?php echo esc_attr($params['rows']); ?>" role="grid" style="width:100%" width="100%">
						<thead>
							<tr class="row100 head">
								<th class="cell100 column2 country_title"><?php echo esc_html($params['country_title']); ?></th>
								<th class="cell100 column3 confirmed_title"><?php echo esc_html($params['confirmed_title']); ?></th>
					...
					<script>
						jQuery(document).ready(function($) {
							...
							function table_row(item){
								...
								var value = '<tr class="row100 body"><td class="cell100 column2 country_title" data-label="'+item.city+'" title="'+item.city+'" ncrts-country="'+item.city+'">'+item.city+'</td>\
									<td class="cell100 column3 confirmed_title" data-label="confirmed_title">'+item.cases+'</td>\
									<td class="cell100 column6 deaths_title" data-label="deaths_title">'+item.deaths+'</td>\
									<td class="cell100 column9 recovered_title" data-label="recovered_title">'+item.recovered+'</td>\
									<td class="cell100 column11 active_title" data-label="active_title">'+item.active+'</td>\
									</td>\
								</tr>';
								return value;
							...
							$('#city_select').on('change', function() {
                                //$('.chan').attr("id",this.value);
                                if ($.fn.DataTable.isDataTable("#<?php echo esc_attr($uniqId); ?>")) {
                                    $('#<?php echo esc_attr($uniqId); ?>').DataTable().clear().destroy();
                                }
								fetch(this.value);
							});
							function fetch(city){
								$.ajax( {
									...
									data : {action: "city_data",'city':city},
									success:function(data) {
										var data = JSON.parse(data);
										var rows = '';
										for(var i=0;i<data.length;i++){
											rows+=table_row(data[i]);
										}
		...
		?><div id="ncrts-admin-container">
				<div class="grid-x grid-container grid-padding-y admin-settings">
					<div class="cell small-12">
						<div class="callout">
							<h2><?php echo esc_html__('COVID-19 Options', 'covid'); ?><span class="v">2.2.6.1</span></h2>
						...
						<div class="tabs-content grid-x" data-tabs-content="setting-tabs">
							<div class="tabs-panel is-active" id="options" role="tabpanel" aria-labelledby="options-label">
										...
										<div class="small-12 cell">
											<select name="covid_country">
												...
												foreach ($data as $item) {
													echo '<option value="' . $item->country . '">' . nycreatisCN($item) . '</option>';
												}
												?>
											</select>
										</div>
								...
								<div class="grid-x display-required callout" style="opacity: 1; pointer-events: inherit;">
									...
									<div class="small-12 cell">
										<div id="covid19">
											<p id="covidsh" class="covid_shortcode"><?php esc_html_e('[COVID19-SHEET country_title="Country" land="" rows="20" search="Search by Country..." confirmed_title="Cases" today_cases="24h" deaths_title="Deaths" today_deaths="24h" recovered_title="Recovered" active_title="Active" tests_title="Tests"]', 'covid'); ?></p>
										</div>
									</div>
								</div>
								...
								<div class="display-required callout primary" style="opacity: 1; pointer-events: inherit;">
									...
									<hr>
									<div class="small-12 cell">
										<h3><?php esc_html_e('ISO 3166 Country List', 'covid'); ?></h3>
									</div>
									<p><?php _e('This is the list of ISO 3166 countries: <a href="http://www.localeplanet.com/icu/iso3166.html" target="_blank">http://www.localeplanet.com/icu/iso3166.html</a>', 'covid'); ?>.</p>
								</div>
...
function covid_add_plugin_page_contact_link( $links ) {
	$links[] = '<a href="http://www.developerharsh.com" target="_blank">' . __('Get Help') . '</a>';
	return $links;
}

function nycreatisCN($c){
	if (!empty($c->countryInfo->iso2) && !empty(COVID_NYCREATIS_CL[$c->countryInfo->iso2])) {
...
function covid_add_plugin_page_settings_link($links){
	$links[] = '<a style="color:#148910" href="' .
		admin_url('options-general.php?page=covid-plugin-options') .
		'">' . __('Settings & Shortcodes') . '</a>';
	return $links;
}
...
function true_option_settings(){
	...
	$true_field_params = array(
		'type'      => 'checkbox',
		'id'        => 'cov_zoom',
		'desc'      => esc_html__('Disable', 'covid'),
		'label_for' => 'cov_zoom'
	);
	add_settings_field('cov_zoom_field', esc_html__('Map zoom', 'covid'), 'true_option_display_settings', $true_page, 'true_section_1', $true_field_params);
